...
Sending while a turn is still running queues the input instead of dropping it — the
placeholder reads `(queued)`, and everything queued runs in order once the turn ends.
Slash commands queue the same way, so `Ctrl+C` remains the way out mid-turn.
